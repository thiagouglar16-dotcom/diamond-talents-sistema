<?php
$tituloPagina = 'Telemarketing';
require_once 'includes/header.php';
$pdo = getDB();
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $a = $_POST['acao'] ?? '';
    if ($a === 'novo' || $a === 'editar') {
        $fields = ["empresa","nome","codigo","email","telefone","situacao"];
        $vals = array_map(fn($f) => $_POST[$f] ?? '', $fields);
        if ($a === 'novo') {
            $cols = implode(',', $fields);
            $phs  = implode(',', array_fill(0, count($fields), '?'));
            $pdo->prepare("INSERT INTO telemarketings ($cols) VALUES ($phs)")->execute($vals);
        } else {
            $set = implode('=?,', $fields) . '=?';
            $vals[] = (int)$_POST['id'];
            $pdo->prepare("UPDATE telemarketings SET $set WHERE id=?")->execute($vals);
        }
    }
    if ($a === 'excluir') $pdo->prepare("DELETE FROM telemarketings WHERE id=?")->execute([(int)$_POST['id']]);
    header('Location: /telemarketings.php'); exit;
}
$busca = trim($_GET['busca'] ?? '');
$ppp = max(1,(int)($_GET['p'] ?? 20)); $pg = max(1,(int)($_GET['pg'] ?? 1)); $off = ($pg-1)*$ppp;
$w = $busca ? "WHERE nome LIKE ?" : '';
$bp = $busca ? ["%$busca%"] : [];
$tot = $pdo->prepare("SELECT COUNT(*) FROM telemarketings $w"); $tot->execute($bp); $total = $tot->fetchColumn();
$stmt = $pdo->prepare("SELECT * FROM telemarketings $w ORDER BY nome ASC LIMIT $ppp OFFSET $off"); $stmt->execute($bp);
$rows = $stmt->fetchAll();
?>
<div class="breadcrumb-bar"><a href="/index.php"><i class="bi bi-house-fill"></i></a> / <span>Telemarketing</span></div>
<div class="d-flex justify-content-between align-items-center mb-3">
  <h2>Gerenciar telemarketing</h2>
...
<div class="modal fade" id="modalForm" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
  ...
  <div class="modal-body"><div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label">Código</label><input type="text" name="codigo" id="fCodigo" class="form-control"></div>
  </div></div>
